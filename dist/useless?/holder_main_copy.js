...
const run = async () => {
    ...
    if (!fs_1.default.existsSync("bob_connection_id.txt")) {
        ...
        const invitationUrl = await question("Paste invitation URL from verifier: ");
        rl.close();
        const connectionRecord = await (0, holder_config_copy_1.receiveInvitation)(bobAgent, invitationUrl);
        if (!connectionRecord) {
