...
class AskarBaseWallet {
        ...
        try {
            ...
            if ((0, utils_1.isKeyTypeSupportedByAskarForPurpose)(keyType, utils_1.AskarKeyTypePurpose.KeyManagement) &&
                ...
                try {
                    ...
                    // Store key
                    await this.withSession((session) => session.insertKey({ key: _key, name: keyId ?? core_1.TypedArrayEncoder.toBase58(keyPublicBytes) }));
                    key.handle.free();
    ...
    async sign({ data, key }) {
        ...
        try {
            if ((0, utils_1.isKeyTypeSupportedByAskarForPurpose)(key.keyType, utils_1.AskarKeyTypePurpose.KeyManagement)) {
                askarKey = await this.withSession(async (session) => (await session.fetchKey({ name: key.publicKeyBase58 }))?.key);
            }
            ...
            if ((0, utils_1.isKeyTypeSupportedByAskarForPurpose)(key.keyType, utils_1.AskarKeyTypePurpose.Signing)) {
                ...
                if (this.signingKeyProviderRegistry.hasProviderForKeyType(key.keyType)) {
                    ...
                    const privateKeyBase58 = keyPair?.privateKeyBase58 ??
                        (askarKey?.secretBytes ? core_1.TypedArrayEncoder.toBase58(askarKey.secretBytes) : undefined);
                    if (!privateKeyBase58) {
                        throw new core_1.WalletError('Key entry not found');
                    }
    ...
    async pack(payload, recipientKeys, senderVerkey // in base58
        ...
        try {
            ...
            const envelope = (0, didcommV1_1.didcommV1Pack)(payload, recipientKeys, senderKey?.key);
            return envelope;
        }
        finally {
    ...
    async directDecryptCompactJweEcdhEs({ compactJwe, recipientKey, }) {
        ...
        if ((0, utils_1.isKeyTypeSupportedByAskarForPurpose)(recipientKey.keyType, utils_1.AskarKeyTypePurpose.KeyManagement)) {
            askarKey = await this.withSession(async (session) => (await session.fetchKey({ name: recipientKey.publicKeyBase58 }))?.key);
        }
        if (!askarKey) {
            throw new core_1.WalletError('Key entry not found');
        }
    ...
    async retrieveKeyPair(publicKeyBase58) {
        try {
            ...
            if (!entryObject)
                return null;
            return core_1.JsonEncoder.fromString(entryObject?.value);
        }
        catch (error) {
